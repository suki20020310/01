...
# 2 "<built-in>" 2
# 1 "../ip/ip6/pooling.cpp" 2
# 1 "../ip/ip6/pooling.h" 1
...
# 2 "../ip/ip6/pooling.cpp" 2
# 15 "../ip/ip6/pooling.cpp"
__attribute__((sdx_kernel("pooling", 0))) void pooling(
...
#line 16 "C:/Users/11400/Desktop/shuffletest/HLS/ip8/solution1/csynth.tcl"
#pragma HLSDIRECTIVE TOP name=pooling
# 28 "../ip/ip6/pooling.cpp"

#line 6 "C:/Users/11400/Desktop/shuffletest/HLS/ip8/solution1/directives.tcl"
#pragma HLSDIRECTIVE TOP name=pooling
# 28 "../ip/ip6/pooling.cpp"
...
#pragma HLS INTERFACE m_axi port=feature_in bundle=gmem0 depth=1024*1024*1024
#pragma HLS INTERFACE m_axi port=feature_out bundle=gmem0 depth=1024*1024*1024


#pragma HLS INTERFACE s_axilite port=return bundle=control
#pragma HLS INTERFACE s_axilite port=C_in bundle=control offset=0x10
#pragma HLS INTERFACE s_axilite port=H_in bundle=control offset=0x14
#pragma HLS INTERFACE s_axilite port=W_in bundle=control offset=0x18
#pragma HLS INTERFACE s_axilite port=kernel_size bundle=control offset=0x1C
#pragma HLS INTERFACE s_axilite port=stride bundle=control offset=0x20
#pragma HLS INTERFACE s_axilite port=padding bundle=control offset=0x24
#pragma HLS INTERFACE s_axilite port=pool_type bundle=control offset=0x28


 VITIS_LOOP_48_1: for (int c = 0; c < C_in; ++c) {

        VITIS_LOOP_50_2: for (int h_out = 0; h_out < H_out; ++h_out) {
            VITIS_LOOP_51_3: for (int w_out = 0; w_out < W_out; ++w_out) {
                if (pool_type == 0) {

                    Dtype_f max_val = -128;


                    VITIS_LOOP_57_4: for (int kh = 0; kh < kernel_size; ++kh) {
                        VITIS_LOOP_58_5: for (int kw = 0; kw < kernel_size; ++kw) {
                            ...
                            if (h_in >= 0 && h_in < H_in && w_in >= 0 && w_in < W_in) {
                                ...
                                int in_offset = (h_in * W_in + w_in) * C_in + c;


                                if (feature_in[in_offset] > max_val) {
                                    max_val = feature_in[in_offset];
                                }
                            }
                        }
                    }
                    ...
                    int out_offset = (h_out * W_out + w_out) * C_in + c;
                    feature_out[out_offset] = max_val;
                } else {
                    ...
                    VITIS_LOOP_85_6: for (int kh = 0; kh < kernel_size; ++kh) {
                        VITIS_LOOP_86_7: for (int kw = 0; kw < kernel_size; ++kw) {
                            ...
                            if (h_in >= 0 && h_in < H_in && w_in >= 0 && w_in < W_in) {
                                ...
                                sum += (Dtype_acc)feature_in[in_offset];
                                count++;
                            }
                        }
                    }
                    ...
                    Dtype_f avg_val = (count > 0) ? (Dtype_f)(sum / count) : Dtype_f(0);
                    int out_offset = (h_out * W_out + w_out) * C_in + c;
                    feature_out[out_offset] = avg_val;
                }
            }
        }
    }
