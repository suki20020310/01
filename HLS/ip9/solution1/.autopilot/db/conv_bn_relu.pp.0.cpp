...
# 2 "<built-in>" 2
# 1 "../ip/ip6/conv_bn_relu.cpp" 2
# 1 "../ip/ip6/conv_bn_relu.h" 1
...
# 5 "../ip/ip6/conv_bn_relu.h" 2


typedef float Dtype_f;
typedef float Dtype_w;
typedef double Dtype_acc;
# 37 "../ip/ip6/conv_bn_relu.h"
__attribute__((sdx_kernel("conv_bn_relu", 0))) void conv_bn_relu(
...
# 3 "../ip/ip6/conv_bn_relu.cpp" 2
# 23 "../ip/ip6/conv_bn_relu.cpp"
__attribute__((sdx_kernel("conv_bn_relu", 0))) void conv_bn_relu(
...
#line 15 "C:/Users/11400/Desktop/shuffletest/HLS/ip9/solution1/csynth.tcl"
#pragma HLSDIRECTIVE TOP name=conv_bn_relu
# 42 "../ip/ip6/conv_bn_relu.cpp"
...
#pragma HLS INTERFACE m_axi depth=2147483647 port=feature_in bundle=gmem1 offset=slave
#pragma HLS INTERFACE m_axi depth=2147483647 port=conv_kernel bundle=gmem2 offset=slave
#pragma HLS INTERFACE m_axi depth=2147483647 port=bn_mean bundle=gmem3 offset=slave
#pragma HLS INTERFACE m_axi depth=2147483647 port=bn_var bundle=gmem3 offset=slave
#pragma HLS INTERFACE m_axi depth=2147483647 port=bn_gamma bundle=gmem3 offset=slave
#pragma HLS INTERFACE m_axi depth=2147483647 port=bn_beta bundle=gmem3 offset=slave


#pragma HLS INTERFACE s_axilite port=return
#pragma HLS INTERFACE s_axilite register port=conv_type offset=0x10
#pragma HLS INTERFACE s_axilite register port=C_in offset=0x14
#pragma HLS INTERFACE s_axilite register port=H_in offset=0x18
#pragma HLS INTERFACE s_axilite register port=W_in offset=0x1C
#pragma HLS INTERFACE s_axilite register port=C_out offset=0x20
#pragma HLS INTERFACE s_axilite register port=kernel_size offset=0x24
#pragma HLS INTERFACE s_axilite register port=stride offset=0x28
#pragma HLS INTERFACE s_axilite register port=padding offset=0x2C
...
#pragma HLS DATAFLOW
 ...
 if (conv_type == 0) {
        ...
        Dtype_acc bn_params[512];
#pragma HLS ARRAY_PARTITION variable=bn_params cyclic factor=4 dim=1


 VITIS_LOOP_79_1: for (int c_out_param = 0; c_out_param < C_out; ++c_out_param) {
#pragma HLS PIPELINE II=1
 Dtype_acc var_eps = (Dtype_acc)bn_var[c_out_param] + 1e-5f;
            Dtype_acc inv_std = 1.0 / sqrt((float)var_eps);
            bn_params[c_out_param] = (Dtype_acc)bn_gamma[c_out_param] * inv_std;
        }


        VITIS_LOOP_87_2: for (int c_out_blk = 0; c_out_blk < C_out; c_out_blk += 8) {

            int c_out_end = (c_out_blk + 8 < (int)C_out) ?
                            (c_out_blk + 8) : (int)C_out;


            VITIS_LOOP_93_3: for (int h_out_blk = 0; h_out_blk < H_out; h_out_blk += 4) {

                int h_out_end = (h_out_blk + 4 < (int)H_out) ?
                                (h_out_blk + 4) : (int)H_out;


                VITIS_LOOP_99_4: for (int w_out_blk = 0; w_out_blk < W_out; w_out_blk += 4) {

                    int w_out_end = (w_out_blk + 4 < (int)W_out) ?
                                    (w_out_blk + 4) : (int)W_out;


                    Dtype_f local_feature_in_blk[4 + 3 - 1][4 + 3 - 1][8] = {0};
                    Dtype_w local_conv_kernel_blk[8][8][3][3] = {0};


#pragma HLS ARRAY_PARTITION variable=local_feature_in_blk cyclic factor=2 dim=3
#pragma HLS ARRAY_PARTITION variable=local_conv_kernel_blk cyclic factor=2 dim=1
#pragma HLS ARRAY_PARTITION variable=local_conv_kernel_blk cyclic factor=2 dim=3
#pragma HLS ARRAY_PARTITION variable=local_conv_kernel_blk complete dim=4


 VITIS_LOOP_115_5: for (int h_in_blk = 0; h_in_blk < h_out_end - h_out_blk + kernel_size - 1; ++h_in_blk) {
                        VITIS_LOOP_116_6: for (int w_in_blk = 0; w_in_blk < w_out_end - w_out_blk + kernel_size - 1; ++w_in_blk) {
                            VITIS_LOOP_117_7: for (int c_in = 0; c_in < C_in; ++c_in) {
#pragma HLS PIPELINE II=1

 int h_in = h_out_blk * stride + h_in_blk - padding;
                                int w_in = w_out_blk * stride + w_in_blk - padding;


                                bool valid_h = (h_in >= 0) && (h_in < H_in);
                                bool valid_w = (w_in >= 0) && (w_in < W_in);
                                if (valid_h && valid_w) {
                                    int in_offset = (h_in * W_in + w_in) * C_in + c_in;
                                    local_feature_in_blk[h_in_blk][w_in_blk][c_in] = feature_in[in_offset];
                                } else {

                                    local_feature_in_blk[h_in_blk][w_in_blk][c_in] = 0;
                                }
                            }
                        }
                    }
                    ...
                    VITIS_LOOP_138_8: for (int c_out = c_out_blk; c_out < c_out_end; ++c_out) {
                        VITIS_LOOP_139_9: for (int c_in = 0; c_in < C_in; ++c_in) {
                            VITIS_LOOP_140_10: for (int kh = 0; kh < kernel_size; ++kh) {
                                VITIS_LOOP_141_11: for (int kw = 0; kw < kernel_size; ++kw) {
#pragma HLS PIPELINE II=1
 int w_offset = c_out * C_in * kernel_size * kernel_size +
                                                  c_in * kernel_size * kernel_size +
                                                  kh * kernel_size + kw;
                                    local_conv_kernel_blk[c_out - c_out_blk][c_in][kh][kw] = conv_kernel[w_offset];
                                }
                            }
                        }
                    }
                    ...
                    VITIS_LOOP_153_12: for (int h_out = h_out_blk; h_out < h_out_end; ++h_out) {
                        VITIS_LOOP_154_13: for (int w_out = w_out_blk; w_out < w_out_end; ++w_out) {
                            VITIS_LOOP_155_14: for (int c_out = c_out_blk; c_out < c_out_end; ++c_out) {
#pragma HLS PIPELINE II=2

 Dtype_acc sum = 0;


                                int h_off = h_out - h_out_blk;
                                int w_off = w_out - w_out_blk;
                                int c_out_off = c_out - c_out_blk;



#pragma HLS LOOP_FLATTEN off
 VITIS_LOOP_168_15: for (int c_in = 0; c_in < C_in; ++c_in) {
                                    VITIS_LOOP_169_16: for (int kh = 0; kh < kernel_size; ++kh) {
                                        VITIS_LOOP_170_17: for (int kw = 0; kw < kernel_size; ++kw) {

                                            sum += (Dtype_acc)local_feature_in_blk[h_off + kh][w_off + kw][c_in] *
                                                   (Dtype_acc)local_conv_kernel_blk[c_out_off][c_in][kh][kw];
                                        }
                                    }
                                }
                                ...
                                val = val * bn_params[c_out];
                                val += (Dtype_acc)bn_beta[c_out];


                                Dtype_f bn_val = (Dtype_f)val;
                                bn_val = (bn_val < 0) ? 0 : bn_val;
...
#pragma HLS ARRAY_PARTITION variable=bn_params cyclic factor=4 dim=1
 ...
 VITIS_LOOP_205_18: for (int c_out_param = 0; c_out_param < C_out; ++c_out_param) {
#pragma HLS PIPELINE II=1
 Dtype_acc var_eps = (Dtype_acc)bn_var[c_out_param] + 1e-5f;
            Dtype_acc inv_std = 1.0 / sqrt((float)var_eps);
            bn_params[c_out_param] = (Dtype_acc)bn_gamma[c_out_param] * inv_std;
        ...
        VITIS_LOOP_213_19: for (int h_out_blk = 0; h_out_blk < H_out; h_out_blk += 4) {

            int h_out_end = (h_out_blk + 4 < (int)H_out) ?
                            (h_out_blk + 4) : (int)H_out;


            VITIS_LOOP_219_20: for (int w_out_blk = 0; w_out_blk < W_out; w_out_blk += 4) {

                int w_out_end = (w_out_blk + 4 < (int)W_out) ?
                                (w_out_blk + 4) : (int)W_out;


                VITIS_LOOP_225_21: for (int c_out = 0; c_out < C_out; ++c_out) {
#pragma HLS PIPELINE II=2

 int c_in = c_out;


                    Dtype_w local_kernel[3][3];
#pragma HLS ARRAY_PARTITION variable=local_kernel cyclic factor=2 dim=1
#pragma HLS ARRAY_PARTITION variable=local_kernel cyclic factor=2 dim=2

 VITIS_LOOP_235_22: for (int kh = 0; kh < kernel_size; ++kh) {
                        VITIS_LOOP_236_23: for (int kw = 0; kw < kernel_size; ++kw) {
                            int w_offset = c_in * kernel_size * kernel_size + kh * kernel_size + kw;
                            local_kernel[kh][kw] = conv_kernel[w_offset];
                        }
                    }


                    VITIS_LOOP_243_24: for (int h_out = h_out_blk; h_out < h_out_end; ++h_out) {
                        VITIS_LOOP_244_25: for (int w_out = w_out_blk; w_out < w_out_end; ++w_out) {

                            Dtype_acc sum = 0;


#pragma HLS LOOP_FLATTEN off
 VITIS_LOOP_250_26: for (int kh = 0; kh < kernel_size; ++kh) {
                                VITIS_LOOP_251_27: for (int kw = 0; kw < kernel_size; ++kw) {
                                    ...
                                    if (valid_h && valid_w) {
                                        int in_offset = (h_in * W_in + w_in) * C_in + c_in;
                                        sum += (Dtype_acc)feature_in[in_offset] * (Dtype_acc)local_kernel[kh][kw];
                                    }
                                }
                            }
