...
typedef struct {
    ...
    XConv_bn_relu_uio_map maps[ MAX_UIO_MAPS ];
} XConv_bn_relu_uio_info;

/***************** Variable Definitions **************************************/
static XConv_bn_relu_uio_info uio_info;
...
static int uio_info_read_name(XConv_bn_relu_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XConv_bn_relu_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XConv_bn_relu_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XConv_bn_relu_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XConv_bn_relu_Initialize(XConv_bn_relu *InstancePtr, const char* InstanceName) {
	XConv_bn_relu_uio_info *InfoPtr = &uio_info;
