...
entity output_conv_pool_bn_control_s_axi is
generic (
...
end entity output_conv_pool_bn_control_s_axi;
...
architecture behave of output_conv_pool_bn_control_s_axi is
    ...
    constant ADDR_C_IN_DATA_0        : INTEGER := 16#10#;
    constant ADDR_C_IN_CTRL          : INTEGER := 16#14#;
    constant ADDR_H_W_DATA_0         : INTEGER := 16#18#;
    constant ADDR_H_W_CTRL           : INTEGER := 16#1c#;
    constant ADDR_IN_R_DATA_0        : INTEGER := 16#20#;
    constant ADDR_IN_R_DATA_1        : INTEGER := 16#24#;
    constant ADDR_IN_R_CTRL          : INTEGER := 16#28#;
    constant ADDR_CONV_KERNEL_DATA_0 : INTEGER := 16#2c#;
    constant ADDR_CONV_KERNEL_DATA_1 : INTEGER := 16#30#;
    constant ADDR_CONV_KERNEL_CTRL   : INTEGER := 16#34#;
    constant ADDR_BN_MEAN_DATA_0     : INTEGER := 16#38#;
    constant ADDR_BN_MEAN_DATA_1     : INTEGER := 16#3c#;
    constant ADDR_BN_MEAN_CTRL       : INTEGER := 16#40#;
    constant ADDR_BN_VAR_DATA_0      : INTEGER := 16#44#;
    constant ADDR_BN_VAR_DATA_1      : INTEGER := 16#48#;
    constant ADDR_BN_VAR_CTRL        : INTEGER := 16#4c#;
    constant ADDR_BN_GAMMA_DATA_0    : INTEGER := 16#50#;
    constant ADDR_BN_GAMMA_DATA_1    : INTEGER := 16#54#;
    constant ADDR_BN_GAMMA_CTRL      : INTEGER := 16#58#;
    constant ADDR_BN_BETA_DATA_0     : INTEGER := 16#5c#;
    constant ADDR_BN_BETA_DATA_1     : INTEGER := 16#60#;
    constant ADDR_BN_BETA_CTRL       : INTEGER := 16#64#;
    constant ADDR_FC_WEIGHTS_DATA_0  : INTEGER := 16#68#;
    constant ADDR_FC_WEIGHTS_DATA_1  : INTEGER := 16#6c#;
    constant ADDR_FC_WEIGHTS_CTRL    : INTEGER := 16#70#;
    constant ADDR_FC_BIASES_DATA_0   : INTEGER := 16#74#;
    constant ADDR_FC_BIASES_DATA_1   : INTEGER := 16#78#;
    constant ADDR_FC_BIASES_CTRL     : INTEGER := 16#7c#;
    constant ADDR_OUT_R_DATA_0       : INTEGER := 16#80#;
    constant ADDR_OUT_R_DATA_1       : INTEGER := 16#84#;
    constant ADDR_OUT_R_CTRL         : INTEGER := 16#88#;
    constant ADDR_C_OUT_DATA_0       : INTEGER := 16#8c#;
    constant ADDR_C_OUT_CTRL         : INTEGER := 16#90#;
    constant ADDR_NUM_CLASSES_DATA_0 : INTEGER := 16#94#;
    constant ADDR_NUM_CLASSES_CTRL   : INTEGER := 16#98#;
    ...
    signal int_h_w             : UNSIGNED(15 downto 0) := (others => '0');
    ...
    signal int_fc_weights      : UNSIGNED(63 downto 0) := (others => '0');
    signal int_fc_biases       : UNSIGNED(63 downto 0) := (others => '0');
    signal int_out_r           : UNSIGNED(63 downto 0) := (others => '0');
    signal int_c_out           : UNSIGNED(15 downto 0) := (others => '0');
    signal int_num_classes     : UNSIGNED(15 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_H_W_DATA_0 =>
                        rdata_data <= RESIZE(int_h_w(15 downto 0), 32);
                    ...
                    when ADDR_FC_WEIGHTS_DATA_0 =>
                        rdata_data <= RESIZE(int_fc_weights(31 downto 0), 32);
                    when ADDR_FC_WEIGHTS_DATA_1 =>
                        rdata_data <= RESIZE(int_fc_weights(63 downto 32), 32);
                    when ADDR_FC_BIASES_DATA_0 =>
                        rdata_data <= RESIZE(int_fc_biases(31 downto 0), 32);
                    when ADDR_FC_BIASES_DATA_1 =>
                        rdata_data <= RESIZE(int_fc_biases(63 downto 32), 32);
                    ...
                    when ADDR_NUM_CLASSES_DATA_0 =>
                        rdata_data <= RESIZE(int_num_classes(15 downto 0), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    fc_weights           <= STD_LOGIC_VECTOR(int_fc_weights);
    fc_biases            <= STD_LOGIC_VECTOR(int_fc_biases);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_H_W_DATA_0) then
                    int_h_w(15 downto 0) <= (UNSIGNED(WDATA(15 downto 0)) and wmask(15 downto 0)) or ((not wmask(15 downto 0)) and int_h_w(15 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_FC_WEIGHTS_DATA_0) then
                    int_fc_weights(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_fc_weights(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_FC_WEIGHTS_DATA_1) then
                    int_fc_weights(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_fc_weights(63 downto 32));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_FC_BIASES_DATA_0) then
                    int_fc_biases(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_fc_biases(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_FC_BIASES_DATA_1) then
                    int_fc_biases(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_fc_biases(63 downto 32));
                end if;
            end if;
        end if;
    end process;
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_NUM_CLASSES_DATA_0) then
                    int_num_classes(15 downto 0) <= (UNSIGNED(WDATA(15 downto 0)) and wmask(15 downto 0)) or ((not wmask(15 downto 0)) and int_num_classes(15 downto 0));
                end if;
            end if;
        end if;
    end process;
