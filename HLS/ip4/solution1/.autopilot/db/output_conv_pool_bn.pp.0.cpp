# 1 "../ip/ip4/output_conv_pool_bn.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 395 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "../ip/ip4/output_conv_pool_bn.cpp" 2
...
# 2 "../ip/ip4/output_conv_pool_bn.cpp" 2
# 1 "../ip/ip4/output_conv_pool_bn.h" 1

# 1 "D:/Xilinx/Vitis_HLS/2022.2/common/technology/autopilot\\ap_int.h" 1
# 3 "../ip/ip4/output_conv_pool_bn.h" 2
# 23 "../ip/ip4/output_conv_pool_bn.h"
typedef ap_int<8> Dtype_f;
typedef ap_int<8> Dtype_w;
typedef ap_int<32> Dtype_acc;


void output_conv_pool_bn(
    const Dtype_f* in,
    const Dtype_w* conv_kernel,
    const Dtype_f* bn_mean,
    const Dtype_f* bn_var,
    const Dtype_f* bn_gamma,
    const Dtype_f* bn_beta,
    Dtype_f* out,
    ap_uint<16> c_in,
    ap_uint<16> c_out,
    ap_uint<16> h_w
);
# 3 "../ip/ip4/output_conv_pool_bn.cpp" 2
# 17 "../ip/ip4/output_conv_pool_bn.cpp"
...
# 49 "../ip/ip4/output_conv_pool_bn.cpp"
...
# 68 "../ip/ip4/output_conv_pool_bn.cpp"
__attribute__((sdx_kernel("output_conv_pool_bn", 0))) void output_conv_pool_bn(
    const Dtype_f* in,
    const Dtype_w* conv_kernel,
    const Dtype_f* bn_mean,
    const Dtype_f* bn_var,
    const Dtype_f* bn_gamma,
    const Dtype_f* bn_beta,
    const Dtype_w* fc_weights,
    const Dtype_w* fc_biases,
    Dtype_f* out,
    ap_uint<16> c_in,
    ap_uint<16> c_out,
    ap_uint<16> h_w,
    ap_uint<16> num_classes
) {
#line 16 "C:/Users/11400/Desktop/shuffle/HLS/ip4/solution1/csynth.tcl"
#pragma HLSDIRECTIVE TOP name=output_conv_pool_bn
# 82 "../ip/ip4/output_conv_pool_bn.cpp"

#line 6 "C:/Users/11400/Desktop/shuffle/HLS/ip4/solution1/directives.tcl"
#pragma HLSDIRECTIVE TOP name=output_conv_pool_bn
# 82 "../ip/ip4/output_conv_pool_bn.cpp"


#pragma HLS INTERFACE s_axilite port=return bundle=control
#pragma HLS INTERFACE m_axi port=in bundle=gmem0 depth=1024*1024*1024
#pragma HLS INTERFACE m_axi port=conv_kernel bundle=gmem0 depth=1024*1024*1024
#pragma HLS INTERFACE m_axi port=bn_mean bundle=gmem0 depth=1024*1024
#pragma HLS INTERFACE m_axi port=bn_var bundle=gmem0 depth=1024*1024
#pragma HLS INTERFACE m_axi port=bn_gamma bundle=gmem0 depth=1024*1024
#pragma HLS INTERFACE m_axi port=bn_beta bundle=gmem0 depth=1024*1024
#pragma HLS INTERFACE m_axi port=fc_weights bundle=gmem0 depth=1024*1024
#pragma HLS INTERFACE m_axi port=fc_biases bundle=gmem0 depth=1024*1024
#pragma HLS INTERFACE m_axi port=out bundle=gmem0 depth=1024*1024
...
#pragma HLS INTERFACE s_axilite port=num_classes bundle=control offset=0x1C




 Dtype_acc mini_accumulator[64];

#pragma HLS BIND_STORAGE variable=mini_accumulator type=RAM_S2P


 ap_uint<32> spatial_size = h_w.to_int() * h_w.to_int();



    Dtype_f conv_features[512];



    VITIS_LOOP_115_1: for (ap_uint<16> c_out_idx = 0; c_out_idx < c_out; c_out_idx++) {
#pragma HLS PIPELINE II=1

 Dtype_acc sum = 0;


        VITIS_LOOP_121_2: for (ap_uint<16> c_in_idx = 0; c_in_idx < c_in; c_in_idx++) {
            VITIS_LOOP_122_3: for (ap_uint<16> h = 0; h < h_w; h++) {
                VITIS_LOOP_123_4: for (ap_uint<16> w = 0; w < h_w; w++) {

                    ap_uint<32> in_idx = c_in_idx.to_int() * h_w.to_int() * h_w.to_int() + h.to_int() * h_w.to_int() + w.to_int();
                    Dtype_f pixel_val = in[in_idx.to_int()];


                    ap_uint<32> kernel_idx = c_out_idx.to_int() * c_in.to_int() + c_in_idx.to_int();
                    Dtype_w weight = conv_kernel[kernel_idx.to_int()];
        ...
        Dtype_acc pool_val = sum / (spatial_size.to_int() * 256);


        conv_features[c_out_idx.to_int()] = unified_batch_norm_relu(
            pool_val,
            bn_mean[c_out_idx.to_int()],
            bn_var[c_out_idx.to_int()],
            bn_gamma[c_out_idx.to_int()],
            bn_beta[c_out_idx.to_int()],
            16
        );


        if (c_out_idx.to_int() < 64) {
            mini_accumulator[c_out_idx.to_int()] = sum;
        }
    }


    VITIS_LOOP_158_5: for (ap_uint<16> class_idx = 0; class_idx < num_classes; class_idx++) {
#pragma HLS PIPELINE II=1
 Dtype_acc fc_sum = 0;


        VITIS_LOOP_163_6: for (ap_uint<16> feat_idx = 0; feat_idx < c_out; feat_idx++) {
#pragma HLS UNROLL factor=4

 ap_uint<32> weight_idx = class_idx.to_int() * c_out.to_int() + feat_idx.to_int();


            fc_sum += conv_features[feat_idx.to_int()] * fc_weights[weight_idx.to_int()];
        }


        fc_sum += fc_biases[class_idx.to_int()];


        out[class_idx.to_int()] = (Dtype_f)(fc_sum >> 8);
    }
}
