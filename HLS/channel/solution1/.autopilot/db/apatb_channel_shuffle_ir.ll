; ModuleID = 'C:/Users/11400/Desktop/shuffletest/HLS/channel/solution1/.autopilot/db/a.g.ld.5.gdce.bc'
...
define void @apatb_channel_shuffle_ir(%"struct.ap_uint<16>"* nocapture readonly %c_in, %"struct.ap_uint<16>"* nocapture readonly %h_w, %"struct.ap_uint<8>"* nocapture readonly %groups, float* noalias nocapture nonnull readonly %data_in, float* noalias nocapture nonnull %data_out) local_unnamed_addr #0 {
entry:
  %malloccall = tail call i8* @malloc(i64 8589934588)
  %data_in_copy = bitcast i8* %malloccall to [2147483647 x float]*
  %malloccall1 = tail call i8* @malloc(i64 8589934588)
  %data_out_copy = bitcast i8* %malloccall1 to [2147483647 x float]*
  %0 = bitcast float* %data_in to [2147483647 x float]*
  %1 = bitcast float* %data_out to [2147483647 x float]*
  call fastcc void @copy_in([2147483647 x float]* nonnull %0, [2147483647 x float]* %data_in_copy, [2147483647 x float]* nonnull %1, [2147483647 x float]* %data_out_copy)
  %2 = getelementptr inbounds [2147483647 x float], [2147483647 x float]* %data_in_copy, i32 0, i32 0
  %3 = getelementptr inbounds [2147483647 x float], [2147483647 x float]* %data_out_copy, i32 0, i32 0
  call void @apatb_channel_shuffle_hw(%"struct.ap_uint<16>"* %c_in, %"struct.ap_uint<16>"* %h_w, %"struct.ap_uint<8>"* %groups, float* %2, float* %3)
  call void @copy_back([2147483647 x float]* %0, [2147483647 x float]* %data_in_copy, [2147483647 x float]* %1, [2147483647 x float]* %data_out_copy)
...
entry:
  %5 = bitcast float* %3 to [2147483647 x float]*
  %6 = bitcast float* %4 to [2147483647 x float]*
  call void @copy_out([2147483647 x float]* null, [2147483647 x float]* %5, [2147483647 x float]* null, [2147483647 x float]* %6)
  %7 = bitcast [2147483647 x float]* %5 to float*
  %8 = bitcast [2147483647 x float]* %6 to float*
  call void @channel_shuffle_hw_stub(%"struct.ap_uint<16>"* %0, %"struct.ap_uint<16>"* %1, %"struct.ap_uint<8>"* %2, float* %7, float* %8)
  call void @copy_in([2147483647 x float]* null, [2147483647 x float]* %5, [2147483647 x float]* null, [2147483647 x float]* %6)
  ret void
}
