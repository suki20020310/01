...
// ==============================================================
`timescale 1ns/1ps
module fully_connected_control_s_axi
#(parameter
    ...
    output wire [15:0]                   in_features,
    output wire [15:0]                   out_features,
...
localparam
    ...
    ADDR_IER                 = 7'h08,
    ADDR_ISR                 = 7'h0c,
    ADDR_IN_FEATURES_DATA_0  = 7'h10,
    ADDR_IN_FEATURES_CTRL    = 7'h14,
    ADDR_OUT_FEATURES_DATA_0 = 7'h18,
    ADDR_OUT_FEATURES_CTRL   = 7'h1c,
    ADDR_INPUT_R_DATA_0      = 7'h20,
    ADDR_INPUT_R_DATA_1      = 7'h24,
    ADDR_INPUT_R_CTRL        = 7'h28,
    ADDR_WEIGHTS_DATA_0      = 7'h2c,
    ADDR_WEIGHTS_DATA_1      = 7'h30,
    ADDR_WEIGHTS_CTRL        = 7'h34,
    ADDR_BIASES_DATA_0       = 7'h38,
    ADDR_BIASES_DATA_1       = 7'h3c,
    ADDR_BIASES_CTRL         = 7'h40,
    ADDR_OUTPUT_R_DATA_0     = 7'h44,
    ADDR_OUTPUT_R_DATA_1     = 7'h48,
    ADDR_OUTPUT_R_CTRL       = 7'h4c,
    WRIDLE                   = 2'd0,
    WRDATA                   = 2'd1,
    WRRESP                   = 2'd2,
    WRRESET                  = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [15:0]                   int_in_features = 'b0;
    reg  [15:0]                   int_out_features = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_IN_FEATURES_DATA_0: begin
                    rdata <= int_in_features[15:0];
                end
                ADDR_OUT_FEATURES_DATA_0: begin
                    rdata <= int_out_features[15:0];
                end
...
assign in_features       = int_in_features;
assign out_features      = int_out_features;
...
// int_in_features[15:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_in_features[15:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_IN_FEATURES_DATA_0)
            int_in_features[15:0] <= (WDATA[31:0] & wmask) | (int_in_features[15:0] & ~wmask);
    end
end

// int_out_features[15:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_out_features[15:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_OUT_FEATURES_DATA_0)
            int_out_features[15:0] <= (WDATA[31:0] & wmask) | (int_out_features[15:0] & ~wmask);
    end
end
