...
// ==============================================================
`timescale 1ns/1ps
module initial_conv_bn_relu_control_s_axi
#(parameter
    ...
    output wire [15:0]                   W_in,
    output wire [7:0]                    K,
    output wire [7:0]                    padding,
    output wire [7:0]                    pool_stride,
    output wire [15:0]                   H_in,
    output wire [15:0]                   C_out,
    output wire [7:0]                    stride,
    output wire [7:0]                    pool_kernel,
    output wire [7:0]                    pool_padding,
...
localparam
    ...
    ADDR_W_IN_DATA_0         = 8'h18,
    ADDR_W_IN_CTRL           = 8'h1c,
    ADDR_K_DATA_0            = 8'h20,
    ADDR_K_CTRL              = 8'h24,
    ADDR_PADDING_DATA_0      = 8'h28,
    ADDR_PADDING_CTRL        = 8'h2c,
    ADDR_POOL_STRIDE_DATA_0  = 8'h30,
    ADDR_POOL_STRIDE_CTRL    = 8'h34,
    ADDR_H_IN_DATA_0         = 8'h38,
    ADDR_H_IN_CTRL           = 8'h3c,
    ADDR_C_OUT_DATA_0        = 8'h40,
    ADDR_C_OUT_CTRL          = 8'h44,
    ADDR_STRIDE_DATA_0       = 8'h48,
    ADDR_STRIDE_CTRL         = 8'h4c,
    ADDR_POOL_KERNEL_DATA_0  = 8'h50,
    ADDR_POOL_KERNEL_CTRL    = 8'h54,
    ADDR_POOL_PADDING_DATA_0 = 8'h58,
    ADDR_POOL_PADDING_CTRL   = 8'h5c,
    ADDR_FEATURE_IN_DATA_0   = 8'h60,
    ADDR_FEATURE_IN_DATA_1   = 8'h64,
    ADDR_FEATURE_IN_CTRL     = 8'h68,
    ADDR_CONV_KERNEL_DATA_0  = 8'h6c,
    ADDR_CONV_KERNEL_DATA_1  = 8'h70,
    ADDR_CONV_KERNEL_CTRL    = 8'h74,
    ADDR_BN_MEAN_DATA_0      = 8'h78,
    ADDR_BN_MEAN_DATA_1      = 8'h7c,
    ADDR_BN_MEAN_CTRL        = 8'h80,
    ADDR_BN_VAR_DATA_0       = 8'h84,
    ADDR_BN_VAR_DATA_1       = 8'h88,
    ADDR_BN_VAR_CTRL         = 8'h8c,
    ADDR_BN_GAMMA_DATA_0     = 8'h90,
    ADDR_BN_GAMMA_DATA_1     = 8'h94,
    ADDR_BN_GAMMA_CTRL       = 8'h98,
    ADDR_BN_BETA_DATA_0      = 8'h9c,
    ADDR_BN_BETA_DATA_1      = 8'ha0,
    ADDR_BN_BETA_CTRL        = 8'ha4,
    ADDR_FEATURE_OUT_DATA_0  = 8'ha8,
    ADDR_FEATURE_OUT_DATA_1  = 8'hac,
    ADDR_FEATURE_OUT_CTRL    = 8'hb0,
    WRIDLE                   = 2'd0,
    WRDATA                   = 2'd1,
    WRRESP                   = 2'd2,
    WRRESET                  = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [15:0]                   int_W_in = 'b0;
    reg  [7:0]                    int_K = 'b0;
    reg  [7:0]                    int_padding = 'b0;
    reg  [7:0]                    int_pool_stride = 'b0;
    reg  [15:0]                   int_H_in = 'b0;
    reg  [15:0]                   int_C_out = 'b0;
    reg  [7:0]                    int_stride = 'b0;
    reg  [7:0]                    int_pool_kernel = 'b0;
    reg  [7:0]                    int_pool_padding = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_POOL_KERNEL_DATA_0: begin
                    rdata <= int_pool_kernel[7:0];
                end
                ADDR_POOL_PADDING_DATA_0: begin
                    rdata <= int_pool_padding[7:0];
                end
...
assign pool_kernel       = int_pool_kernel;
assign pool_padding      = int_pool_padding;
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_W_IN_DATA_0)
            int_W_in[15:0] <= (WDATA[31:0] & wmask) | (int_W_in[15:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_H_IN_DATA_0)
            int_H_in[15:0] <= (WDATA[31:0] & wmask) | (int_H_in[15:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_pool_kernel[7:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_POOL_KERNEL_DATA_0)
            int_pool_kernel[7:0] <= (WDATA[31:0] & wmask) | (int_pool_kernel[7:0] & ~wmask);
    end
end

// int_pool_padding[7:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_pool_padding[7:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_POOL_PADDING_DATA_0)
            int_pool_padding[7:0] <= (WDATA[31:0] & wmask) | (int_pool_padding[7:0] & ~wmask);
    end
end
