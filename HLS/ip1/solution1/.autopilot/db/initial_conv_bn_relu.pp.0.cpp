# 1 "../ip/Ip1/initial_conv_bn_relu.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 395 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "../ip/Ip1/initial_conv_bn_relu.cpp" 2
...
# 2 "../ip/Ip1/initial_conv_bn_relu.cpp" 2
# 1 "../ip/Ip1/initial_conv_bn_relu.h" 1

# 1 "D:/Xilinx/Vitis_HLS/2022.2/common/technology/autopilot\\ap_int.h" 1
# 3 "../ip/Ip1/initial_conv_bn_relu.h" 2
...
__attribute__((sdx_kernel("initial_conv_bn_relu", 0))) void initial_conv_bn_relu(
    ...
    ap_uint<8> stride,
    ap_uint<8> padding,
    ap_uint<8> pool_kernel,
    ap_uint<8> pool_stride,
    ap_uint<8> pool_padding,
...
# 3 "../ip/Ip1/initial_conv_bn_relu.cpp" 2
# 18 "../ip/Ip1/initial_conv_bn_relu.cpp"
__attribute__((sdx_kernel("initial_conv_bn_relu", 0))) void initial_conv_bn_relu(
...
#line 16 "C:/Users/11400/Desktop/shuffle/HLS/ip1/solution1/csynth.tcl"
#pragma HLSDIRECTIVE TOP name=initial_conv_bn_relu
# 39 "../ip/Ip1/initial_conv_bn_relu.cpp"

#line 6 "C:/Users/11400/Desktop/shuffle/HLS/ip1/solution1/directives.tcl"
#pragma HLSDIRECTIVE TOP name=initial_conv_bn_relu
# 39 "../ip/Ip1/initial_conv_bn_relu.cpp"
...
#pragma HLS INTERFACE s_axilite register port=C_in offset=0x10
#pragma HLS INTERFACE s_axilite register port=H_in offset=0x14
#pragma HLS INTERFACE s_axilite register port=W_in offset=0x18
#pragma HLS INTERFACE s_axilite register port=C_out offset=0x1C
#pragma HLS INTERFACE s_axilite register port=K offset=0x20
#pragma HLS INTERFACE s_axilite register port=stride offset=0x24
#pragma HLS INTERFACE s_axilite register port=padding offset=0x28
#pragma HLS INTERFACE s_axilite register port=pool_kernel offset=0x2C
#pragma HLS INTERFACE s_axilite register port=pool_stride offset=0x30
#pragma HLS INTERFACE s_axilite register port=pool_padding offset=0x34


 ap_uint<16> H_pool_out = ((H_in + 2 * padding - K) / stride + 1 + 2 * pool_padding - pool_kernel) / pool_stride + 1;
    ap_uint<16> W_pool_out = ((W_in + 2 * padding - K) / stride + 1 + 2 * pool_padding - pool_kernel) / pool_stride + 1;





    VITIS_LOOP_70_1: for (int h_pool = 0; h_pool < H_pool_out; ++h_pool) {
        VITIS_LOOP_71_2: for (int w_pool = 0; w_pool < W_pool_out; ++w_pool) {
            VITIS_LOOP_72_3: for (int c_out = 0; c_out < C_out; ++c_out) {

                Dtype_f max_val = -128;


                VITIS_LOOP_77_4: for (int p_idx = 0; p_idx < 4; ++p_idx) {
                    int ph = p_idx / 2;
                    int pw = p_idx % 2;


                    int conv_h = h_pool * 2 + ph;
                    int conv_w = w_pool * 2 + pw;

                    if (conv_h >= 0 && conv_w >= 0 && conv_h < H_in && conv_w < W_in) {

                        Dtype_acc sum = 0;


                        VITIS_LOOP_90_5: for (int ck = 0; ck < C_in*K*K; ++ck) {
                            int c_in = ck / (K*K);
                            int k = ck % (K*K);
                            int kh = k / K;
                            int kw = k % K;
                            ...
                            if (in_h >= 0 && in_h < H_in && in_w >= 0 && in_w < W_in) {
                                ...
                                int in_offset = (in_h * W_in + in_w) * C_in + c_in;
                                int w_offset = c_out * C_in*K*K + c_in*K*K + k;
                        ...
                        val = val * (Dtype_acc)bn_gamma[c_out] / (Dtype_acc)bn_var[c_out];
                        val += (Dtype_acc)bn_beta[c_out];


                        Dtype_f bn_relu_val = (val > 0) ? (Dtype_f)(val / 256) : Dtype_f(0);


                        if (bn_relu_val > max_val) {
                            max_val = bn_relu_val;
                        }
                    }
                }
                ...
                feature_out[h_pool * W_pool_out * C_out + w_pool * C_out + c_out] = max_val;
            }
        }
    }
}
