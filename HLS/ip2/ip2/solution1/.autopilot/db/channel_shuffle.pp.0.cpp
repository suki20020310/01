...
# 2 "<built-in>" 2
# 1 "../../ip/ip2/channel_shuffle.cpp" 2
...
# 2 "../../ip/ip2/channel_shuffle.cpp" 2
# 1 "../../ip/ip2/channel_shuffle.h" 1

# 1 "D:/Xilinx/Vitis_HLS/2022.2/common/technology/autopilot\\ap_int.h" 1
# 3 "../../ip/ip2/channel_shuffle.h" 2
...
__attribute__((sdx_kernel("shufflenet_block", 0))) void shufflenet_block(
    const Dtype_f* in,
...
# 3 "../../ip/ip2/channel_shuffle.cpp" 2
# 17 "../../ip/ip2/channel_shuffle.cpp"
...
#pragma HLS INTERFACE m_axi port=in bundle=gmem_in depth=1024*1024*1024
#pragma HLS INTERFACE m_axi port=out bundle=gmem_out depth=1024*1024*1024
...
#pragma HLS INTERFACE s_axilite port=h_w bundle=control offset=0x18
#pragma HLS INTERFACE s_axilite port=groups bundle=control offset=0x24
#pragma HLS INTERFACE s_axilite port=return bundle=control


#pragma HLS INLINE off

 const ap_uint<16> group_ch = c_in / groups;


    VITIS_LOOP_81_1: for (ap_uint<16> h = 0; h < h_w; h++) {
        VITIS_LOOP_82_2: for (ap_uint<16> w = 0; w < h_w; w++) {
#pragma HLS PIPELINE II=1
#pragma HLS LOOP_TRIPCOUNT min=1 max=512

 VITIS_LOOP_86_3: for (ap_uint<8> g = 0; g < groups; g++) {
#pragma HLS UNROLL factor=2
 VITIS_LOOP_88_4: for (ap_uint<16> c = 0; c < group_ch; c++) {
                    ap_uint<32> in_idx = (g * group_ch + c) * h_w * h_w + h * h_w + w;
                    ap_uint<16> out_c = c * groups + g;
                    ap_uint<32> out_idx = out_c * h_w * h_w + h * h_w + w;
...
#line 16 "C:/Users/11400/Desktop/shuffle/HLS/ip2/ip2/solution1/csynth.tcl"
#pragma HLSDIRECTIVE TOP name=shufflenet_block
# 124 "../../ip/ip2/channel_shuffle.cpp"

#line 6 "C:/Users/11400/Desktop/shuffle/HLS/ip2/ip2/solution1/directives.tcl"
#pragma HLSDIRECTIVE TOP name=shufflenet_block
# 124 "../../ip/ip2/channel_shuffle.cpp"


#pragma HLS INTERFACE m_axi port=in bundle=gmem_in depth=1024*1024*1024
#pragma HLS INTERFACE m_axi port=conv1_kernel bundle=gmem_weights1 depth=1024*1024*1024
#pragma HLS INTERFACE m_axi port=dw_kernel bundle=gmem_weights2 depth=1024*1024*1024
#pragma HLS INTERFACE m_axi port=conv2_kernel bundle=gmem_weights3 depth=1024*1024*1024
#pragma HLS INTERFACE m_axi port=bn1_mean bundle=gmem_bn1 depth=1024*1024
#pragma HLS INTERFACE m_axi port=bn1_var bundle=gmem_bn1 depth=1024*1024
#pragma HLS INTERFACE m_axi port=bn1_gamma bundle=gmem_bn1 depth=1024*1024
#pragma HLS INTERFACE m_axi port=bn1_beta bundle=gmem_bn1 depth=1024*1024
#pragma HLS INTERFACE m_axi port=bn2_mean bundle=gmem_bn2 depth=1024*1024
#pragma HLS INTERFACE m_axi port=bn2_var bundle=gmem_bn2 depth=1024*1024
#pragma HLS INTERFACE m_axi port=bn2_gamma bundle=gmem_bn2 depth=1024*1024
#pragma HLS INTERFACE m_axi port=bn2_beta bundle=gmem_bn2 depth=1024*1024
#pragma HLS INTERFACE m_axi port=bn3_mean bundle=gmem_bn3 depth=1024*1024
#pragma HLS INTERFACE m_axi port=bn3_var bundle=gmem_bn3 depth=1024*1024
#pragma HLS INTERFACE m_axi port=bn3_gamma bundle=gmem_bn3 depth=1024*1024
#pragma HLS INTERFACE m_axi port=bn3_beta bundle=gmem_bn3 depth=1024*1024
#pragma HLS INTERFACE m_axi port=out bundle=gmem_out depth=1024*1024*1024
#pragma HLS INTERFACE s_axilite port=c_in bundle=control offset=0x10
#pragma HLS INTERFACE s_axilite port=c_out bundle=control offset=0x14
#pragma HLS INTERFACE s_axilite port=h_w bundle=control offset=0x18
#pragma HLS INTERFACE s_axilite port=c_reduce bundle=control offset=0x1C
#pragma HLS INTERFACE s_axilite port=stride bundle=control offset=0x20
#pragma HLS INTERFACE s_axilite port=return bundle=control


 ap_uint<16> c_in_half = c_in / 2;


    const int MINI_BUFFER_SIZE = 64;
    ap_int<8> mini_buffer[MINI_BUFFER_SIZE];
#pragma HLS BIND_STORAGE variable=mini_buffer type=RAM_S2P
 ...
 VITIS_LOOP_162_1: for (ap_uint<16> h = 0; h < h_w; h++) {
        VITIS_LOOP_163_2: for (ap_uint<16> w = 0; w < h_w; w++) {
#pragma HLS PIPELINE II=1
 VITIS_LOOP_165_3: for (ap_uint<16> c = c_in_half; c < c_in; c++) {

                ap_uint<32> in_idx = c * h_w * h_w + h * h_w + w;

                ap_uint<32> out_idx = c * h_w * h_w + h * h_w + w;
                out[out_idx] = in[in_idx];
            }
        }
    }
    ...
    static Dtype_f conv_branch[64*16];
#pragma HLS BIND_STORAGE variable=conv_branch type=RAM_S2P
 ...
 VITIS_LOOP_188_4: for (ap_uint<16> h = 0; h < h_w; h++) {
        VITIS_LOOP_189_5: for (ap_uint<16> w = 0; w < h_w; w++) {
#pragma HLS PIPELINE II=1
#pragma HLS LOOP_TRIPCOUNT min=1 max=512

 VITIS_LOOP_193_6: for (ap_uint<16> cr_group = 0; cr_group < c_reduce; cr_group += 16) {
                ap_uint<16> cr_end = (cr_group + 16 > c_reduce) ? c_reduce : ap_uint<16>(cr_group + 16);

                VITIS_LOOP_196_7: for (ap_uint<16> cr = cr_group; cr < cr_end; cr++) {

                    Dtype_acc conv1_sum = 0;

                    VITIS_LOOP_200_8: for (ap_uint<16> c_in_conv = 0; c_in_conv < c_in_half; c_in_conv++) {
                        ap_uint<32> kernel_idx = cr * c_in_half + c_in_conv;
                        ap_uint<32> in_idx = c_in_conv * h_w * h_w + h * h_w + w;
                        conv1_sum += in[in_idx] * conv1_kernel[kernel_idx];
                    }
                    Dtype_acc conv1_val = conv1_sum >> 8;


                    ap_uint<16> buf_idx = (cr - cr_group) * h_w * h_w + h * h_w + w;
                    conv_branch[buf_idx % 64*16] = unified_batch_norm_relu(
                        conv1_val,
                        bn1_mean[0],
                        bn1_var[0],
                        bn1_gamma[0],
    ...
    VITIS_LOOP_228_9: for (ap_uint<16> h = 0; h < h_w; h++) {
        VITIS_LOOP_229_10: for (ap_uint<16> w = 0; w < h_w; w++) {
#pragma HLS PIPELINE II=1
#pragma HLS LOOP_TRIPCOUNT min=1 max=512

 VITIS_LOOP_233_11: for (ap_uint<16> cr_group = 0; cr_group < c_reduce; cr_group += 16) {
                ap_uint<16> cr_end = (cr_group + 16 > c_reduce) ? c_reduce : ap_uint<16>(cr_group + 16);

                VITIS_LOOP_236_12: for (ap_uint<16> cr = cr_group; cr < cr_end; cr++) {

                    Dtype_acc dw_sum = 0;

                    ap_uint<16> buf_idx_base = (cr - cr_group) * h_w * h_w;


                    if (h > 0 && w > 0) {
                        ap_uint<16> buf_idx = buf_idx_base + (h-1) * h_w + (w-1);
                        dw_sum += conv_branch[buf_idx % 64*16] * dw_kernel[cr * 9 + 0];
                    }
                    if (h > 0) {
                        ap_uint<16> buf_idx = buf_idx_base + (h-1) * h_w + w;
                        dw_sum += conv_branch[buf_idx % 64*16] * dw_kernel[cr * 9 + 1];
                    }
                    if (h > 0 && w < h_w-1) {
                        ap_uint<16> buf_idx = buf_idx_base + (h-1) * h_w + (w+1);
                        dw_sum += conv_branch[buf_idx % 64*16] * dw_kernel[cr * 9 + 2];
                    }
                    if (w > 0) {
                        ap_uint<16> buf_idx = buf_idx_base + h * h_w + (w-1);
                        dw_sum += conv_branch[buf_idx % 64*16] * dw_kernel[cr * 9 + 3];
                    }

                    ap_uint<16> buf_idx = buf_idx_base + h * h_w + w;
                    dw_sum += conv_branch[buf_idx % 64*16] * dw_kernel[cr * 9 + 4];
                    if (w < h_w-1) {
                        ap_uint<16> buf_idx = buf_idx_base + h * h_w + (w+1);
                        dw_sum += conv_branch[buf_idx % 64*16] * dw_kernel[cr * 9 + 5];
                    }
                    if (h < h_w-1 && w > 0) {
                        ap_uint<16> buf_idx = buf_idx_base + (h+1) * h_w + (w-1);
                        dw_sum += conv_branch[buf_idx % 64*16] * dw_kernel[cr * 9 + 6];
                    }
                    if (h < h_w-1) {
                        ap_uint<16> buf_idx = buf_idx_base + (h+1) * h_w + w;
                        dw_sum += conv_branch[buf_idx % 64*16] * dw_kernel[cr * 9 + 7];
                    }
                    if (h < h_w-1 && w < h_w-1) {
                        ap_uint<16> buf_idx = buf_idx_base + (h+1) * h_w + (w+1);
                        dw_sum += conv_branch[buf_idx % 64*16] * dw_kernel[cr * 9 + 8];
                    }

                    Dtype_acc dw_val = dw_sum >> 8;


                    Dtype_acc bn2_val = unified_batch_norm(
                        dw_val,
                        bn2_mean[0],
                        bn2_var[0],
                        bn2_gamma[0],
                    ...
                    conv_branch[buf_idx % 64*16] = bn2_val >> 8;
                }
            }
        }
    }
    ...
    VITIS_LOOP_297_13: for (ap_uint<16> c = 0; c < c_in_half; c++) {
        VITIS_LOOP_298_14: for (ap_uint<16> h = 0; h < h_w; h++) {
            VITIS_LOOP_299_15: for (ap_uint<16> w = 0; w < h_w; w++) {
#pragma HLS PIPELINE II=1
 ap_uint<32> out_idx = c * h_w * h_w + h * h_w + w;
                out[out_idx] = 0;
            }
    ...
    VITIS_LOOP_309_16: for (ap_uint<16> h = 0; h < h_w; h++) {
        VITIS_LOOP_310_17: for (ap_uint<16> w = 0; w < h_w; w++) {
#pragma HLS PIPELINE II=1
#pragma HLS LOOP_TRIPCOUNT min=1 max=512

 VITIS_LOOP_314_18: for (ap_uint<16> c_out_group = 0; c_out_group < c_in_half; c_out_group += 4) {
                ap_uint<16> c_out_end = (c_out_group + 4 > c_in_half) ? c_in_half : ap_uint<16>(c_out_group + 4);


                VITIS_LOOP_318_19: for (ap_uint<16> c_out_conv = c_out_group; c_out_conv < c_out_end; c_out_conv++) {
                    ap_uint<32> out_idx = c_out_conv * h_w * h_w + h * h_w + w;
                    out[out_idx] = 0;
                }


                VITIS_LOOP_324_20: for (ap_uint<16> cr_group = 0; cr_group < c_reduce; cr_group += 16) {
                    ap_uint<16> cr_end = (cr_group + 16 > c_reduce) ? c_reduce : ap_uint<16>(cr_group + 16);

                    VITIS_LOOP_327_21: for (ap_uint<16> cr = cr_group; cr < cr_end; cr++) {

                        ap_uint<16> buf_idx = (cr - cr_group) * h_w * h_w + h * h_w + w;
                        Dtype_f in_val = conv_branch[buf_idx % 64*16];


                        VITIS_LOOP_333_22: for (ap_uint<16> c_out_conv = c_out_group; c_out_conv < c_out_end; c_out_conv++) {
                            ap_uint<32> kernel_idx = c_out_conv * c_reduce + cr;
                            ap_uint<32> out_idx = c_out_conv * h_w * h_w + h * h_w + w;
                            out[out_idx] += in_val * conv2_kernel[kernel_idx];
                        }
                    }
                }


                VITIS_LOOP_342_23: for (ap_uint<16> c_out_conv = c_out_group; c_out_conv < c_out_end; c_out_conv++) {
                    ap_uint<32> out_idx = c_out_conv * h_w * h_w + h * h_w + w;
                    Dtype_acc conv2_val = out[out_idx] >> 8;
...
#pragma HLS BIND_STORAGE variable=shuffle_temp type=RAM_S2P
 ...
 VITIS_LOOP_372_24: for (ap_uint<16> h = 0; h < h_w; h++) {
        VITIS_LOOP_373_25: for (ap_uint<16> w = 0; w < h_w; w++) {
#pragma HLS PIPELINE II=1
#pragma HLS LOOP_TRIPCOUNT min=1 max=512

 VITIS_LOOP_377_26: for (ap_uint<16> block = 0; block < c_out; block += 32) {
                ap_uint<16> block_end = (block + 32 > c_out) ? c_out : ap_uint<16>(block + 32);
                ap_uint<16> block_size = block_end - block;


                VITIS_LOOP_382_27: for (ap_uint<16> c = block; c < block_end; c++) {
                    ap_uint<32> src_idx = c * h_w * h_w + h * h_w + w;
                    shuffle_temp[c - block] = out[src_idx];
                }


                VITIS_LOOP_388_28: for (ap_uint<16> c = block; c < block_end; c++) {

                    ap_uint<16> group_idx = c / group_size;
                    ap_uint<16> in_group_idx = c % group_size;
                    ap_uint<16> new_group_idx = (group_idx + 1) % 2;
                    ap_uint<16> new_c = new_group_idx * group_size + in_group_idx;


                    if (new_c >= block && new_c < block_end) {
                        ap_uint<32> dst_idx = c * h_w * h_w + h * h_w + w;
                        out[dst_idx] = shuffle_temp[new_c - block];
            }
        }
    }
}
