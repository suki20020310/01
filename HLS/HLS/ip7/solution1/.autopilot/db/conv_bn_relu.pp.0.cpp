...
#line 16 "C:/Users/11400/Desktop/shuffletest/HLS/ip7/solution1/csynth.tcl"
#pragma HLSDIRECTIVE TOP name=conv_bn_relu
# 36 "../ip/ip6/conv_bn_relu.cpp"

#line 6 "C:/Users/11400/Desktop/shuffletest/HLS/ip7/solution1/directives.tcl"
#pragma HLSDIRECTIVE TOP name=conv_bn_relu
# 36 "../ip/ip6/conv_bn_relu.cpp"
    ...
    Dtype_f local_feature_in;
    Dtype_w local_conv_kernel;
    Dtype_w local_bn_mean;
    Dtype_w local_bn_var;
    Dtype_w local_bn_gamma;
    Dtype_w local_bn_beta;
...
#pragma HLS INTERFACE s_axilite register port=padding offset=0x2C
 ...
 if (conv_type == NORMAL_CONV) {
        ...
        VITIS_LOOP_72_1: for (int h_out = 0; h_out < H_out; ++h_out) {

            VITIS_LOOP_74_2: for (int w_out = 0; w_out < W_out; ++w_out) {

                VITIS_LOOP_76_3: for (int c_out = 0; c_out < C_out; ++c_out) {
#pragma HLS PIPELINE II=1
 ...
 Dtype_acc sum = 0;
                    ...
                    VITIS_LOOP_83_4: for (int c_in = 0; c_in < C_in; ++c_in) {
#pragma HLS UNROLL factor=4
 VITIS_LOOP_85_5: for (int kh = 0; kh < kernel_size; ++kh) {
#pragma HLS UNROLL
 VITIS_LOOP_87_6: for (int kw = 0; kw < kernel_size; ++kw) {
                    ...
                    local_bn_mean = bn_mean[c_out];
                    local_bn_var = bn_var[c_out];
                    local_bn_gamma = bn_gamma[c_out];
                    local_bn_beta = bn_beta[c_out];


                    Dtype_acc val = sum - (Dtype_acc)local_bn_mean;

                    Dtype_acc eps = 1e-5;

                    val = val * (Dtype_acc)local_bn_gamma / (Dtype_acc)sqrt((float)((Dtype_acc)local_bn_var + eps));
                    val += (Dtype_acc)local_bn_beta;


                    Dtype_f bn_val = (Dtype_f)val ;

                    if (bn_val < 0) bn_val = 0;
        ...
        VITIS_LOOP_137_7: for (int h_out = 0; h_out < H_out; ++h_out) {

            VITIS_LOOP_139_8: for (int w_out = 0; w_out < W_out; ++w_out) {

                VITIS_LOOP_141_9: for (int c_out = 0; c_out < C_out; ++c_out) {
#pragma HLS PIPELINE II=1
 ...
 Dtype_acc sum = 0;
                    ...
                    VITIS_LOOP_150_10: for (int kh = 0; kh < kernel_size; ++kh) {
#pragma HLS UNROLL
 VITIS_LOOP_152_11: for (int kw = 0; kw < kernel_size; ++kw) {
