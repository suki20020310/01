...
entity pooling_control_s_axi is
generic (
...
end entity pooling_control_s_axi;
...
architecture behave of pooling_control_s_axi is
    ...
    constant ADDR_POOL_TYPE_DATA_0   : INTEGER := 16#28#;
    constant ADDR_POOL_TYPE_CTRL     : INTEGER := 16#2c#;
    constant ADDR_H_IN_DATA_0        : INTEGER := 16#30#;
    constant ADDR_H_IN_CTRL          : INTEGER := 16#34#;
    constant ADDR_KERNEL_SIZE_DATA_0 : INTEGER := 16#38#;
    constant ADDR_KERNEL_SIZE_CTRL   : INTEGER := 16#3c#;
    constant ADDR_PADDING_DATA_0     : INTEGER := 16#40#;
    constant ADDR_PADDING_CTRL       : INTEGER := 16#44#;
    constant ADDR_FEATURE_IN_DATA_0  : INTEGER := 16#48#;
    constant ADDR_FEATURE_IN_DATA_1  : INTEGER := 16#4c#;
    constant ADDR_FEATURE_IN_CTRL    : INTEGER := 16#50#;
    constant ADDR_FEATURE_OUT_DATA_0 : INTEGER := 16#54#;
    constant ADDR_FEATURE_OUT_DATA_1 : INTEGER := 16#58#;
    ...
    signal int_pool_type       : UNSIGNED(0 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_POOL_TYPE_DATA_0 =>
                        rdata_data <= RESIZE(int_pool_type(0 downto 0), 32);
...
-- ----------------------- Register logic ----------------
    ...
    pool_type            <= STD_LOGIC_VECTOR(int_pool_type);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_POOL_TYPE_DATA_0) then
                    int_pool_type(0 downto 0) <= (UNSIGNED(WDATA(0 downto 0)) and wmask(0 downto 0)) or ((not wmask(0 downto 0)) and int_pool_type(0 downto 0));
                end if;
            end if;
        end if;
    end process;
