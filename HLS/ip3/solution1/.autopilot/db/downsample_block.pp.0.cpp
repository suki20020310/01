...
# 2 "<built-in>" 2
# 1 "../ip/ip3/downsample_block.cpp" 2
...
# 2 "../ip/ip3/downsample_block.cpp" 2
# 1 "../ip/ip3/downsample_block.h" 1

# 1 "D:/Xilinx/Vitis_HLS/2022.2/common/technology/autopilot\\ap_int.h" 1
# 3 "../ip/ip3/downsample_block.h" 2
...
__attribute__((sdx_kernel("downsample_block", 0))) void downsample_block(
    const Dtype_f* in,
    const Dtype_w* dw_kernel,
    const Dtype_w* conv1_kernel,
    const Dtype_w* conv2_kernel,
    const Dtype_f* bn1_mean,
    const Dtype_f* bn1_var,
    const Dtype_f* bn1_gamma,
    const Dtype_f* bn1_beta,
    const Dtype_f* bn2_mean,
    const Dtype_f* bn2_var,
    const Dtype_f* bn2_gamma,
    const Dtype_f* bn2_beta,
    const Dtype_f* bn3_mean,
    const Dtype_f* bn3_var,
    const Dtype_f* bn3_gamma,
    const Dtype_f* bn3_beta,
    Dtype_f* out,
    ap_uint<16> c_in,
    ap_uint<16> c_out,
    ap_uint<16> h_in,
...
# 3 "../ip/ip3/downsample_block.cpp" 2
# 17 "../ip/ip3/downsample_block.cpp"
Dtype_acc unified_batch_norm(Dtype_acc input_val, Dtype_acc mean, Dtype_acc var, Dtype_acc gamma, Dtype_acc beta) {

    Dtype_acc var_term = 128 + var;



    Dtype_acc inv_var = (1 << 16) / var_term;



    Dtype_acc bn_val = (input_val - mean) * gamma;
    bn_val = (bn_val * inv_var) >> 16;
...
# 49 "../ip/ip3/downsample_block.cpp"
Dtype_f unified_batch_norm_relu(Dtype_acc input_val, Dtype_acc mean, Dtype_acc var, Dtype_acc gamma, Dtype_acc beta, int right_shift_bits = 8) {

    Dtype_acc bn_val = unified_batch_norm(input_val, mean, var, gamma, beta);


    return (bn_val > 0) ? (Dtype_f)(bn_val >> right_shift_bits) : Dtype_f(0);
}
# 65 "../ip/ip3/downsample_block.cpp"
__attribute__((sdx_kernel("downsample_block", 0))) void downsample_block(
    const Dtype_f* in,
    const Dtype_w* dw_kernel,
    const Dtype_w* conv1_kernel,
...
#line 16 "C:/Users/11400/Desktop/shuffle/HLS/ip3/solution1/csynth.tcl"
#pragma HLSDIRECTIVE TOP name=downsample_block
# 90 "../ip/ip3/downsample_block.cpp"

#line 6 "C:/Users/11400/Desktop/shuffle/HLS/ip3/solution1/directives.tcl"
#pragma HLSDIRECTIVE TOP name=downsample_block
# 90 "../ip/ip3/downsample_block.cpp"


#pragma HLS INTERFACE m_axi port=in bundle=gmem2 depth=1024*1024*1024
#pragma HLS INTERFACE m_axi port=dw_kernel bundle=gmem2 depth=1024*1024*1024
#pragma HLS INTERFACE m_axi port=conv1_kernel bundle=gmem2 depth=1024*1024*1024
#pragma HLS INTERFACE m_axi port=conv2_kernel bundle=gmem2 depth=1024*1024*1024
#pragma HLS INTERFACE m_axi port=bn1_mean bundle=gmem2 depth=1024*1024
#pragma HLS INTERFACE m_axi port=bn1_var bundle=gmem2 depth=1024*1024
#pragma HLS INTERFACE m_axi port=bn1_gamma bundle=gmem2 depth=1024*1024
#pragma HLS INTERFACE m_axi port=bn1_beta bundle=gmem2 depth=1024*1024
#pragma HLS INTERFACE m_axi port=bn2_mean bundle=gmem2 depth=1024*1024
#pragma HLS INTERFACE m_axi port=bn2_var bundle=gmem2 depth=1024*1024
#pragma HLS INTERFACE m_axi port=bn2_gamma bundle=gmem2 depth=1024*1024
#pragma HLS INTERFACE m_axi port=bn2_beta bundle=gmem2 depth=1024*1024
#pragma HLS INTERFACE m_axi port=bn3_mean bundle=gmem2 depth=1024*1024
#pragma HLS INTERFACE m_axi port=bn3_var bundle=gmem2 depth=1024*1024
#pragma HLS INTERFACE m_axi port=bn3_gamma bundle=gmem2 depth=1024*1024
#pragma HLS INTERFACE m_axi port=bn3_beta bundle=gmem2 depth=1024*1024
#pragma HLS INTERFACE m_axi port=out bundle=gmem2 depth=1024*1024*1024
#pragma HLS INTERFACE s_axilite port=c_in bundle=control offset=0x10
#pragma HLS INTERFACE s_axilite port=c_out bundle=control offset=0x14
#pragma HLS INTERFACE s_axilite port=h_in bundle=control offset=0x18
#pragma HLS INTERFACE s_axilite port=w_in bundle=control offset=0x1C
#pragma HLS INTERFACE s_axilite port=h_out bundle=control offset=0x20
#pragma HLS INTERFACE s_axilite port=w_out bundle=control offset=0x24
#pragma HLS INTERFACE s_axilite port=stride bundle=control offset=0x28
#pragma HLS INTERFACE s_axilite port=return bundle=control
 ...
 ap_int<8> mini_buffer[32];
#pragma HLS BIND_STORAGE variable=mini_buffer type=RAM_S2P


 ap_uint<16> c_out_half = c_out / 2;
    ...
    VITIS_LOOP_135_1: for (ap_uint<16> h = 0; h < h_out; h++) {
        VITIS_LOOP_136_2: for (ap_uint<16> w = 0; w < w_out; w++) {
#pragma HLS PIPELINE II=1
#pragma HLS LOOP_TRIPCOUNT min=1 max=512

 VITIS_LOOP_140_3: for (ap_uint<16> c_out_group = 0; c_out_group < c_out_half; c_out_group += 16) {
                ap_uint<16> c_out_end = (c_out_group + 16 > c_out_half) ? c_out_half : (ap_uint<16>)(c_out_group + 16);
                ap_uint<16> group_size = c_out_end - c_out_group;


                VITIS_LOOP_145_4: for (ap_uint<16> i = 0; i < group_size; i++) {
                    mini_buffer[i] = 0;
                }


                VITIS_LOOP_150_5: for (ap_uint<16> c_in_conv = 0; c_in_conv < c_in; c_in_conv++) {

                    ap_uint<32> in_idx = c_in_conv * h_in * w_in + h * stride * w_in + w * stride;
                    Dtype_f in_val = in[in_idx];


                    VITIS_LOOP_156_6: for (ap_uint<16> c_out_conv = c_out_group; c_out_conv < c_out_end; c_out_conv++) {
                        ap_uint<16> buf_idx = c_out_conv - c_out_group;

                        ap_uint<32> kernel_idx = c_out_conv * c_in + c_in_conv;
                        Dtype_w weight = conv1_kernel[kernel_idx];
                        mini_buffer[buf_idx] += in_val * weight;
                    }
                }


                VITIS_LOOP_166_7: for (ap_uint<16> c_out_conv = c_out_group; c_out_conv < c_out_end; c_out_conv++) {
                    ap_uint<16> buf_idx = c_out_conv - c_out_group;
                    Dtype_acc conv_val = mini_buffer[buf_idx];


                    Dtype_f conv_result = unified_batch_norm_relu(
                        conv_val,
                        bn1_mean[c_out_conv],
                        bn1_var[c_out_conv],
                        bn1_gamma[c_out_conv],
                        bn1_beta[c_out_conv],
                        8
                    );


                    ap_uint<8> temp_buf_idx = (c_out_conv - c_out_group) % 32;
                    mini_buffer[temp_buf_idx] = conv_result;
                }
            }
        }
    }
    ...
    VITIS_LOOP_189_8: for (ap_uint<16> h = 0; h < h_out; h++) {
        VITIS_LOOP_190_9: for (ap_uint<16> w = 0; w < w_out; w++) {
#pragma HLS PIPELINE II=1
#pragma HLS LOOP_TRIPCOUNT min=1 max=512

 VITIS_LOOP_194_10: for (ap_uint<16> c_group = 0; c_group < c_out_half; c_group += 16) {
                ap_uint<16> c_end = (c_group + 16 > c_out_half) ? c_out_half : (ap_uint<16>)(c_group + 16);

                VITIS_LOOP_197_11: for (ap_uint<16> c = c_group; c < c_end; c++) {
                    ...
                    if (h > 0 && w > 0) {
                        ...
                        ap_uint<16> orig_c = c_group + ((h-1) * w_out + (w-1)) % (c_end - c_group);
                        ap_uint<8> buf_idx = (orig_c - c_group) % 32;
                        sum += mini_buffer[buf_idx] * dw_kernel[c * 9 + 0];
                    }

                    if (h > 0) {
                        ap_uint<16> orig_c = c_group + ((h-1) * w_out + w) % (c_end - c_group);
                        ap_uint<8> buf_idx = (orig_c - c_group) % 32;
                        sum += mini_buffer[buf_idx] * dw_kernel[c * 9 + 1];
                    }

                    if (h > 0 && w < w_out-1) {
                        ap_uint<16> orig_c = c_group + ((h-1) * w_out + (w+1)) % (c_end - c_group);
                        ap_uint<8> buf_idx = (orig_c - c_group) % 32;
                        sum += mini_buffer[buf_idx] * dw_kernel[c * 9 + 2];
                    }

                    if (w > 0) {
                        ap_uint<16> orig_c = c_group + (h * w_out + (w-1)) % (c_end - c_group);
                        ap_uint<8> buf_idx = (orig_c - c_group) % 32;
                        sum += mini_buffer[buf_idx] * dw_kernel[c * 9 + 3];
                    }


                    ap_uint<8> buf_idx = (c - c_group) % 32;
                    sum += mini_buffer[buf_idx] * dw_kernel[c * 9 + 4];

                    if (w < w_out-1) {
                        ap_uint<16> orig_c = c_group + (h * w_out + (w+1)) % (c_end - c_group);
                        ap_uint<8> buf_idx = (orig_c - c_group) % 32;
                        sum += mini_buffer[buf_idx] * dw_kernel[c * 9 + 5];
                    }

                    if (h < h_out-1 && w > 0) {
                        ap_uint<16> orig_c = c_group + ((h+1) * w_out + (w-1)) % (c_end - c_group);
                        ap_uint<8> buf_idx = (orig_c - c_group) % 32;
                        sum += mini_buffer[buf_idx] * dw_kernel[c * 9 + 6];
                    }

                    if (h < h_out-1) {
                        ap_uint<16> orig_c = c_group + ((h+1) * w_out + w) % (c_end - c_group);
                        ap_uint<8> buf_idx = (orig_c - c_group) % 32;
                        sum += mini_buffer[buf_idx] * dw_kernel[c * 9 + 7];
                    }

                    if (h < h_out-1 && w < w_out-1) {
                        ap_uint<16> orig_c = c_group + ((h+1) * w_out + (w+1)) % (c_end - c_group);
                        ap_uint<8> buf_idx = (orig_c - c_group) % 32;
                        sum += mini_buffer[buf_idx] * dw_kernel[c * 9 + 8];
                    }


                    Dtype_acc dw_val = sum >> 8;


                    Dtype_acc bn_val = unified_batch_norm(
                        dw_val,
                        bn2_mean[c],
                        bn2_var[c],
                        bn2_gamma[c],
                    ...
                    buf_idx = (c - c_group) % 32;
                    mini_buffer[buf_idx] = bn_val >> 8;
                }
            }
        }
    }
    ...
    VITIS_LOOP_281_12: for (ap_uint<16> c_out_group = 0; c_out_group < c_out_half; c_out_group += 8) {
        ap_uint<16> c_out_end = (c_out_group + 8 > c_out_half) ? c_out_half : (ap_uint<16>)(c_out_group + 8);


        VITIS_LOOP_285_13: for (ap_uint<16> h = 0; h < h_out; h++) {
            VITIS_LOOP_286_14: for (ap_uint<16> w = 0; w < w_out; w++) {
#pragma HLS PIPELINE II=1
#pragma HLS LOOP_TRIPCOUNT min=1 max=512

 VITIS_LOOP_290_15: for (ap_uint<16> c_out_conv = c_out_group; c_out_conv < c_out_end; c_out_conv++) {

                    ap_uint<32> out_idx = c_out_conv * h_out * w_out + h * w_out + w;
                    out[out_idx] = 0;
                }


                VITIS_LOOP_297_16: for (ap_uint<16> c_in_conv = 0; c_in_conv < c_out_half; c_in_conv++) {

                    ap_uint<8> buf_idx = c_in_conv % 32;
                    Dtype_f in_val = mini_buffer[buf_idx];


                    VITIS_LOOP_303_17: for (ap_uint<16> c_out_conv = c_out_group; c_out_conv < c_out_end; c_out_conv++) {

                        ap_uint<32> kernel_idx = c_out_conv * c_out_half + c_in_conv;
                        Dtype_w weight = conv2_kernel[kernel_idx];
                        ap_uint<32> out_idx = c_out_conv * h_out * w_out + h * w_out + w;
                        out[out_idx] += in_val * weight;
                    }
                }


                VITIS_LOOP_313_18: for (ap_uint<16> c_out_conv = c_out_group; c_out_conv < c_out_end; c_out_conv++) {

                    ap_uint<32> out_idx = c_out_conv * h_out * w_out + h * w_out + w;
                    Dtype_acc conv_val = out[out_idx] >> 8;
                    out[out_idx] = unified_batch_norm_relu(
                        conv_val,
                        bn3_mean[c_out_conv],
                        bn3_var[c_out_conv],
                        bn3_gamma[c_out_conv],
                        bn3_beta[c_out_conv],
                        8
                    );
                }
...
# 337 "../ip/ip3/downsample_block.cpp"
    VITIS_LOOP_337_19: for (ap_uint<16> h = 0; h < h_out; h++) {
        VITIS_LOOP_338_20: for (ap_uint<16> w = 0; w < w_out; w++) {
#pragma HLS PIPELINE II=1
#pragma HLS LOOP_TRIPCOUNT min=1 max=512

 VITIS_LOOP_342_21: for (ap_uint<16> c_out_group = 0; c_out_group < c_out_half; c_out_group += 8) {
                ap_uint<16> c_out_end = (c_out_group + 8 > c_out_half) ? c_out_half : (ap_uint<16>)(c_out_group + 8);


                VITIS_LOOP_346_22: for (ap_uint<16> c_out_conv = c_out_group; c_out_conv < c_out_end; c_out_conv++) {

                    ap_uint<32> out_idx = (c_out_conv + c_out_half) * h_out * w_out + h * w_out + w;
                    out[out_idx] = 0;
                }


                VITIS_LOOP_353_23: for (ap_uint<16> c_in_conv = 0; c_in_conv < c_in; c_in_conv++) {

                    Dtype_acc sum_dw = 0;



                    ap_uint<16> in_h = h * stride;
                    ap_uint<16> in_w = w * stride;


                    if (in_h > 0 && in_w > 0) {
                        ap_uint<32> in_idx = c_in_conv * h_in * w_in + (in_h - 1) * w_in + (in_w - 1);
                        sum_dw += in[in_idx] * dw_kernel[c_in_conv * 9 + 0];
                    }

                    if (in_h > 0) {
                        ap_uint<32> in_idx = c_in_conv * h_in * w_in + (in_h - 1) * w_in + in_w;
                        sum_dw += in[in_idx] * dw_kernel[c_in_conv * 9 + 1];
                    }

                    if (in_h > 0 && in_w < w_in - 1) {
                        ap_uint<32> in_idx = c_in_conv * h_in * w_in + (in_h - 1) * w_in + (in_w + 1);
                        sum_dw += in[in_idx] * dw_kernel[c_in_conv * 9 + 2];
                    }

                    if (in_w > 0) {
                        ap_uint<32> in_idx = c_in_conv * h_in * w_in + in_h * w_in + (in_w - 1);
                        sum_dw += in[in_idx] * dw_kernel[c_in_conv * 9 + 3];
                    }


                    ap_uint<32> in_idx = c_in_conv * h_in * w_in + in_h * w_in + in_w;
                    sum_dw += in[in_idx] * dw_kernel[c_in_conv * 9 + 4];

                    if (in_w < w_in - 1) {
                        ap_uint<32> in_idx = c_in_conv * h_in * w_in + in_h * w_in + (in_w + 1);
                        sum_dw += in[in_idx] * dw_kernel[c_in_conv * 9 + 5];
                    }

                    if (in_h < h_in - 1 && in_w > 0) {
                        ap_uint<32> in_idx = c_in_conv * h_in * w_in + (in_h + 1) * w_in + (in_w - 1);
                        sum_dw += in[in_idx] * dw_kernel[c_in_conv * 9 + 6];
                    }

                    if (in_h < h_in - 1) {
                        ap_uint<32> in_idx = c_in_conv * h_in * w_in + (in_h + 1) * w_in + in_w;
                        sum_dw += in[in_idx] * dw_kernel[c_in_conv * 9 + 7];
                    }

                    if (in_h < h_in - 1 && in_w < w_in - 1) {
                        ap_uint<32> in_idx = c_in_conv * h_in * w_in + (in_h + 1) * w_in + (in_w + 1);
                        sum_dw += in[in_idx] * dw_kernel[c_in_conv * 9 + 8];
                    }


                    Dtype_acc dw_val = sum_dw >> 8;
                    Dtype_acc bn_val = unified_batch_norm(
                        dw_val,
                        bn2_mean[c_in_conv],
                        bn2_var[c_in_conv],
                    ...
                    Dtype_f bn_result = bn_val >> 8;


                    VITIS_LOOP_419_24: for (ap_uint<16> c_out_conv = c_out_group; c_out_conv < c_out_end; c_out_conv++) {

                        ap_uint<32> kernel_idx = c_out_conv * c_in + c_in_conv;
                        Dtype_w weight = conv2_kernel[kernel_idx];
                        ap_uint<32> out_idx = (c_out_conv + c_out_half) * h_out * w_out + h * w_out + w;
                        out[out_idx] += bn_result * weight;
                    }
            }
        }
    }
    ...
    VITIS_LOOP_432_25: for (ap_uint<16> c_out_group = 0; c_out_group < c_out_half; c_out_group += 8) {
        ap_uint<16> c_out_end = (c_out_group + 8 > c_out_half) ? c_out_half : (ap_uint<16>)(c_out_group + 8);

        VITIS_LOOP_435_26: for (ap_uint<16> h = 0; h < h_out; h++) {
            VITIS_LOOP_436_27: for (ap_uint<16> w = 0; w < w_out; w++) {
#pragma HLS PIPELINE II=1
#pragma HLS LOOP_TRIPCOUNT min=1 max=512
 VITIS_LOOP_439_28: for (ap_uint<16> c_out_conv = c_out_group; c_out_conv < c_out_end; c_out_conv++) {
    ...
    ap_uint<16> group_size = c_out / 2;


    Dtype_f shuffle_temp[32];
#pragma HLS BIND_STORAGE variable=shuffle_temp type=RAM_S2P


 VITIS_LOOP_466_29: for (ap_uint<16> h = 0; h < h_out; h++) {
        VITIS_LOOP_467_30: for (ap_uint<16> w = 0; w < w_out; w++) {
#pragma HLS PIPELINE II=1
#pragma HLS LOOP_TRIPCOUNT min=1 max=512

 VITIS_LOOP_471_31: for (ap_uint<16> block = 0; block < c_out; block += 32) {
                ap_uint<16> block_end = (block + 32 > c_out) ? c_out : (ap_uint<16>)(block + 32);
                ap_uint<16> block_size = block_end - block;



                ap_uint<16> mid_point = c_out / 2;


                if (block < mid_point && block_end > mid_point) {

                    ap_uint<16> left_size = mid_point - block;
                    ap_uint<16> right_size = block_end - mid_point;


                    VITIS_LOOP_486_32: for (ap_uint<16> c = 0; c < left_size; c++) {
                        ap_uint<32> src_idx = (block + c) * h_out * w_out + h * w_out + w;
                        shuffle_temp[c] = out[src_idx];
                    }


                    VITIS_LOOP_492_33: for (ap_uint<16> c = 0; c < right_size; c++) {
                        ap_uint<32> src_idx = (mid_point + c) * h_out * w_out + h * w_out + w;
                        shuffle_temp[left_size + c] = out[src_idx];
                    }


                    VITIS_LOOP_498_34: for (ap_uint<16> c = 0; c < left_size; c++) {
                        ap_uint<32> dst_idx = (block + c) * h_out * w_out + h * w_out + w;
                        out[dst_idx] = shuffle_temp[left_size + c];
                    }

                    VITIS_LOOP_503_35: for (ap_uint<16> c = 0; c < right_size; c++) {
                        ap_uint<32> dst_idx = (mid_point + c) * h_out * w_out + h * w_out + w;
                        out[dst_idx] = shuffle_temp[c];
                    }
                }
            }
        }
